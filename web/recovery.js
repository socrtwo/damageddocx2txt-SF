/*
 * Damaged DOCX → Text — pure-browser text salvage and repair for corrupt
 * Microsoft Word .docx (Word 2007+) files.
 *
 * Strategy:
 ...
 *   4. Pull readable text out of word/document.xml (paragraph by paragraph).
 *   5. Repackage all recovered entries as a fresh, well-formed .docx zip.
 *
...
(() => {
  ...
  // Pull readable text from word/document.xml (and any header/footer parts).
  // Each <w:p> paragraph becomes one line; <w:tab/> -> tab, <w:br/>/<w:cr/>
  // -> newline; text comes from <w:t> runs.
  function extractDocxText(entries) {
    const docParts = Object.keys(entries)
      .filter(p => /^word\/(document\d*|header\d*|footer\d*|footnotes|endnotes)\.xml$/i.test(p))
      .sort((a, b) => {
        // document.xml first, then headers/footers/notes.
        const rank = (n) => /document/i.test(n) ? 0 : 1;
        return rank(a) - rank(b) || a.localeCompare(b);
      });
    const paragraphs = [];
    for (const part of docParts) {
      const xml = new TextDecoder('utf-8', { fatal: false }).decode(entries[part]);
      ...
      while ((pm = pRe.exec(xml))) {
        any = true;
        let body = pm[1];
        body = body.replace(/<w:tab\b[^>]*\/?>/g, '\t');
        body = body.replace(/<w:(br|cr)\b[^>]*\/?>/g, '\n');
        const runs = [...body.matchAll(/<w:t\b[^>]*>([\s\S]*?)<\/w:t>/g)].map(m => m[1]);
        paragraphs.push(decodeEntities(runs.join('')));
      }
      // Fallback: if no <w:p> structure survived, grab raw <w:t> runs.
      if (!any) {
        const runs = [...xml.matchAll(/<w:t\b[^>]*>([\s\S]*?)<\/w:t>/g)].map(m => decodeEntities(m[1]));
  ...
  async function buildZip(entries) {
    ...
    return new Blob([...localChunks, ...cdChunks, eocd], { type: DOCX_MIME });
  }
  ...
  async function recover(file, log) {
    ...
    if (typeof S2FileID !== 'undefined' && typeof document !== 'undefined') {
      try {
        const idReport = S2FileID.analyze(bytes, { programKey: 'damageddocx2txt', fileName: file.name });
        S2FileID.renderPanel(document.getElementById('fileid'), idReport);
        log.info(`Identified: ${idReport.primary.description}`);
        if (idReport.mismatch) log.warn('This tool handles .docx — the file appears to be a different type. See the recommendation above.');
    ...
    const paragraphs = extractDocxText(entries);
  ...
  function ready() {
    ...
    $('dl-docx').addEventListener('click', () => {
      if (lastResult) downloadBlob(lastResult.recovered, baseName + '.recovered.docx');
    });
    ...
    async function handle(file) {
      ...
      try {
        ...
        $('dl-docx').disabled = result.entriesCount === 0;
        if (result.paragraphsCount === 0 && result.entriesCount === 0) {
          setStatus('Could not recover anything from this file.', 'error');
        } else {
          setStatus(`Recovered ${result.paragraphsCount} paragraphs of text from ${result.entriesCount} entries.`, 'success');
        }
      } catch (err) {
        console.error(err);
